...
class common(object):
    ...
    def cross_entropy_with_softmax(label, z):
        prob = common.softmax(z)
        loss = common.cross_entropy(label, prob)
        return loss
    ...
    def P(W_new, theta, a_last, W, b, z, u,rho):
        temp = W_new - W
        res = common.eq1(a_last, W, b, z, u,rho) + np.sum(common.eq1_W(a_last, W, b, z, u,rho) * temp) + np.sum(theta * temp * temp) / 2
        return res
    # return the quadratic approximation of a-subproblem
    def Q(a_new, tau, a, W_next, b_next, z_next, u_next,v,z,rho):
        temp = a_new - a
        res = common.a_obj(a, W_next, b_next, z_next, u_next,v,z,rho) + np.sum(common.a_obj_gradient(a, W_next, b_next, z_next, u_next,v,z,rho) * temp) + np.sum(
            tau * temp * temp) / 2
        return res
    # return the objective value of a-subproblem
    def a_obj(a, W_next, b_next, z_next, u_next,v,z,rho):
        res=common.eq1(a, W_next, b_next, z_next, u_next,rho)+rho/2*np.sum((a-common.relu(z)+v)*(a-common.relu(z)+v))
        return res
    # return the gradient of a-subproblem
    def a_obj_gradient(a, W_next, b_next, z_next, u_next,v,z,rho):
        res=common.eq1_a(a, W_next, b_next, z_next, u_next,rho)+rho*(a-common.relu(z)+v)
        return res
    # return the result of W-subproblem
    def update_W(a_last, b, z, W_old, u,rho):
        gradients = common.eq1_W(a_last, W_old, b, z, u,rho)
        gamma = 2
        alpha = 1
        zeta = W_old - gradients / alpha
        while (common.eq1(a_last, zeta, b, z, u,rho) > common.P(zeta, alpha, a_last, W_old, b, z, u,rho)):
            alpha = alpha * gamma
    ...
    def update_b(a_last, W, z, b_old, u,rho):
        gradients = common.eq1_b(a_last, W, b_old, z, u,rho)
        res = b_old - gradients / rho
        return res
    # return the objective value of z-subproblem
    def z_obj(a_last, W, b, z, u,v,a,rho):
        f=(z-mul(W,a_last)-b+u/rho)*(z-mul(W,a_last)-b+u/rho)+(a-common.relu(z)+v)*(a-common.relu(z)+v)
    # return the result of W-subproblem
    def update_W(a_last, b, z, W_old, u,rho):
        gradients = common.eq1_W(a_last, W_old, b, z, u,rho)
        gamma = 2
    ...
    def update_z(a_last, W, b, a,z_old, u,v,rho):
        ...
        z1=np.minimum(z1,0)
        z2=np.maximum(z2,0)
        value1=common.z_obj(a_last, W, b, z1, u,v,a,rho)
        value2=common.z_obj(a_last, W, b, z2, u,v,a,rho)
        z=z1
    ...
    def update_zl(a_last, W, b, label, zl_old, u,rho):
        ...
        for i in range(MAX_ITER):
            fzl_old = fzl
            fzl = common.cross_entropy_with_softmax(label, zl)+rho/2*np.sum((zl-mul(W,a_last)-b+u/rho)*(zl-mul(W,a_last)-b+u/rho))
            if abs(fzl - fzl_old) < TOLERANCE:
                break
            lamda_old = lamda
            lamda = (1 + np.sqrt(1 + 4 * lamda * lamda)) / 2
            gamma = (1 - lamda_old) / lamda
            gradients2 = (common.softmax(zl) - label)
            zeta_old = zeta
    ...
    def update_a(W_next, b_next, z_next, z, a_old, u_next,v,rho):
        gradient = common.a_obj_gradient(a_old, W_next, b_next, z_next, u_next, v,z,rho)
        eta = 2
        t=1
        beta=a_old-gradient/t
        while (common.a_obj(beta, W_next, b_next, z_next, u_next,v,z,rho) > common.Q(beta, t, a_old, W_next, b_next, z_next, u_next,v,z,rho)):
            t = t * eta
            beta =a_old-gradient/t
        tau = t
        a = beta
